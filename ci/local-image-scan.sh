...
docker build -q -f worker/Dockerfile -t iam-worker:scan . >/dev/null || { echo "iam-worker build failed"; exit 1; }

for img in iam-core:scan iam-worker:scan iam-web:scan; do
  echo "== Trivy: $img"
